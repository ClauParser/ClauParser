...
namespace wiz {
	...
	private:
			...
			long long*& _token_arr, long long& _token_arr_size, const LoadDataOption& option)
		{
			...
			if (thr_num == 1) {
				_Scanning(text, start[0], last[0] - start[0], tokens, token_arr_size[0], option);
			}
			else {
			...
			for (long long i = 0; i < token_count; ++i) {
				...
				if (isToken2) {
					...
					if (len >= 2 && text[idx] == '*' && text[idx + 1] == '/') {
						if (len > 2) {
							tokens[real_token_arr_count] = Get(idx + 2, len - 2, text[idx + 2], option);
							real_token_arr_count++;
						}
					}
					else {
	...
	public:
		static int Merge(UserType* next, UserType* ut, UserType** ut_next)
		{
			...
			while (ut->GetIListSize() >= 1 && ut->GetUserTypeListSize() >= 1
				&& (ut->GetUserTypeList(0)->GetName() == "#")) // add usertype to bool isVirtual?
			{
			...
			for (long long i = 0; i < token_arr_len; ++i) {
				...
				case 0:
				{
					...
					if (len == 1 && (-1 != Equal(TYPE_LEFT, GetType(token_arr[i])))) {
						...
						UserType temp(std::string(buffer + GetIdx(var), GetLength(var)));
						var = 0;
					...
					// Right 2
						...
						if (x < token_arr + token_arr_len - 1) {
							...
							if (_len == 1 && -1 != Equal(TYPE_ASSIGN, GetType(token_arr[i + 1]))) {
							...
							else if (_len == 1 && -1 != Equal(TYPE_LEFT, GetType(token_arr[i + 1]))) {
								var = token_arr[i];
							}
							else {
								// val
								if (x <= token_arr + token_arr_len - 1) {
									...
									varVec.push_back(check_syntax_error1(0, err));
			...
			for (long long a = last; a >= start; --a) {
				...
				if (a < last && pass == false) {
					...
					{ // NOT
						...
						if (!(len == 1 && -1 != Equal(TYPE_LEFT, val))) {
							return a;
						}
					}
				}
